...
- **1. PDF 업로드**  
  FastAPI를 통해 사용자가 PDF 파일을 업로드합니다.
- **2. 텍스트/이미지 추출**  
  PyMuPDF를 활용해 각 페이지의 텍스트와 이미지 바이트를 추출합니다.
- **3. 중요 이미지 판별**  
  Vision LLM(GPT 기반)을 통해 이미지가 차트인지 여부와 페이지 내 차지 비율을 기준으로 필터링합니다.
- **비전 모델 분석 결과 + 텍스트 → LLM 전달**  
  추출된 이미지 설명과 텍스트를 LLM(GPT 기반)에 입력하여 대본 생성을 수행합니다.
- **페이지별 발표 대본 생성**  
  전체 문서 내용과 이전 페이지 요약 내용을 함께 참고하여, 흐름이 자연스러운 발표 대본을 자동으로 생성합니다.
