...
- 사용자가 FastAPI를 통해 PDF 파일을 업로드  
- → PyMuPDF로 각 페이지의 텍스트 및 이미지 추출  
- → Vision LLM(GPT 기반)으로 이미지 중요도 판단 (차트 여부, 비율 등)  
- → 이미지 설명 + 텍스트를 PageScript LLM에 입력  
- → 전체 문서 흐름을 고려해 페이지별 발표 대본 자동 생성
...
- 사용자가 선택한 **성별**에 따라 다른 Google TTS 음성(Wavenet-A 또는 C) 사용  
- → 발표 대본에서 강조할 **키워드 자동 추출** (임베딩 유사도 기반)  
- → 강조 키워드 및 대문자 단어에 대해 SSML 변환 적용  
- → SSML 기반으로 Google TTS API 호출, `.wav` 음성 파일 생성  
- → 결과 음성을 base64로 인코딩하여 반환
