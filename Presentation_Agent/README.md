...
- **1. PDF 업로드**: FastAPI를 통해 사용자가 PDF 파일을 업로드합니다.
- **2. 텍스트/이미지 추출**: PyMuPDF를 활용해 각 페이지의 텍스트와 이미지 바이트를 추출합니다.
- **3. 중요 이미지 판별**: Vision LLM(GPT 기반)을 통해 이미지가 차트인지 여부와 페이지 내 차지 비율을 기준으로 필터링합니다.
- **비전 모델 분석 결과 + 텍스트 → LLM 전달**: 추출된 이미지 설명과 텍스트를 LLM(GPT 기반)에 입력하여 대본 생성을 수행합니다.
- **페이지별 발표 대본 생성**: 전체 문서 내용과 이전 페이지 요약 내용을 함께 참고하여, 흐름이 자연스러운 발표 대본을 자동으로 생성합니다.

**TTS 음성 변환 파이프라인 개발 (Google TTS)**  
- 사용자가 선택한 **성별(TTS 음색)**에 따라 다른 Google TTS 음성(Wavenet-A 또는 Wavenet-C)을 사용합니다.
- 페이지별 대본에서 **강조할 키워드**를 벡터 유사도 기반으로 자동 추출하여 SSML 태그로 강조합니다.
- **대문자 단어(API, GPT 등)**는 철자 읽기 처리(`<say-as>`)로 변환하여 자연스러운 발음을 지원합니다.
- SSML 변환 후 Google TTS API에 요청해 `.wav` 파일을 생성하고, base64 인코딩된 결과를 반환합니다.
- 기존 오디오 파일은 요청마다 초기화하여 디렉토리 충돌을 방지합니다.

> 페이지별 스크립트 → 키워드 강조 SSML 변환 → Google TTS → `.wav` 생성 → base64 반환  

**FastAPI를 통한 백엔드 서버 구축 및 API 개발**  
- `POST /generate-script`  
  PDF에서 텍스트와 이미지를 추출하고, LLM을 통해 발표 대본을 생성합니다.
- `POST /generate-audio`  
  생성된 대본을 기반으로 페이지별 음성을 생성합니다.
- `POST /export-presentation`  
  PDF를 PPTX로 변환하고 음성을 삽입해 ZIP 파일로 제공합니다.
---
## 🛤️ 프로젝트 과정
