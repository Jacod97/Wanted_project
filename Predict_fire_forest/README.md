...
### **한계점:**    
먼저, 산불은 발생 자체가 매우 드문 사건이다 보니 대부분의 모델이 '산불 없음'으로 예측하는 경향을 보였습니다. 이로 인해 정확도(accuracy)는 높게 나타났지만, 실제로 산불이 발생한 경우를 제대로 잡아내지 못하는 한계가 있었습니다.   
성능을 정확하게 측정하기 위해선 Test Set을 구축할 때 산불이 발생하는 조건에서의 데이터를 높은 비중으로 구축한다면 조금 더 신뢰할 수 있는 지표가 생길 것이라 판단됩니다. 
    
또한, 국내 산불은 자연 발화보다는 입산자에 의한 실화가 주요 원인이었는데, 입산자의 활동이나 행태를 반영할 수 있는 데이터를 확보하지 못해 보다 정밀한 예측이 어려웠습니다.   
...
Logistic Regression, XGBoost, LightGBM 등의 다양한 모델의 예측 결과를 메타 모델에 통합하여
단일 모델보다 더 강건한 성능을 도출할 수 있도록 구조를 개선할 예정입니다.

이와 함께, 학습에 활용되는 입력 피처(Feature)의 다양성을 확대할 필요도 있습니다.
예를 들어, 지형 및 토지 이용과 관련된 정보로는 경사도, 고도, 토양 유형, 수분량 등을 고려하고,
인간 활동을 반영하는 정보로는 불법 소각, 쓰레기 투기, 캠핑장 및 등산로 분포 같은 데이터를 추가하여
보다 실제 상황에 가까운 예측 모델을 구축할 수 있도록 개선해나갈 계획입니다.
